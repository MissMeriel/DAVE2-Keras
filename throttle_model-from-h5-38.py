...
def parse_args():
    ...
    parser.add_argument("-p", '--path_to_trainingdir', type=str, default='/p/sdbb/BeamNG_DAVE2_racetracks',
    ...
    print(args.path_to_trainingdir, args.noisevar, args.outdir_id, args.img_dims, args.lr, args.epochs, args.batch, args.pretrained_model, args.start_epochs, flush=True)
    return args
...
def main_pytorch_model():
    ...
    dataset = MultiDirectoryDataSequence(args.path_to_trainingdir, image_size=(input_shape[::-1]), 
    ...
    newdir = f"./{model._get_name()}-{input_shape[0]}x{input_shape[1]}-{int(dataset.get_total_samples()/1000)}samples-{args.epochs}epoch-{args.outdir_id}-{timestr}-{randstr}"
    if not os.path.exists(newdir):
        # original_umask = os.umask(0)
        os.mkdir(newdir,  mode=0o777)
        shutil.copyfile(__file__, f"{newdir}/{__file__.split('/')[-1]}", follow_symlinks=False)
        shutil.copyfile("BaseDatasetGenerator.py", f"{newdir}/BaseDatasetGenerator.py", follow_symlinks=False)
    iteration = f'{model._get_name()}-randomblurnoise-{input_shape[0]}x{input_shape[1]}-lr1e4-{args.epochs}epoch-{args.batch}batch-lossMSE-{int(dataset.get_total_samples()/1000)}Ksamples'
